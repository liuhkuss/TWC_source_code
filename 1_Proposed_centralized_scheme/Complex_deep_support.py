...
def state_output(user,constellation):      
    ...
    for index,x in enumerate(user.candidate):
        if x == -1:     
            ...
            # distance = copy.deepcopy(user.distance)
            neighborhood = []
            distance = []

            for index_neighbor,x_neighbor in enumerate(user.neighborhood):
                if x_neighbor.traffic == 'ON':
                    neighborhood.append(x_neighbor)
                    distance.append(user.distance[index_neighbor])
            ...
            for i in range(PS.neighborhood_num):
                if len(neighborhood) == 0:
                    ...
                    neihgbor = neighborhood[0]              
                    assert neihgbor.traffic == 'ON', print('neighbor.traffic =',neihgbor.traffic)
                    d = distance[0] / PS.Rr
                    ...
                    if index != -1:     
                        ...
                        if neihgbor.elevation[index][1] != 0.0:
                            ...
                            if neihgbor.elevation[index][2] != 0.0:
                                theta_a = (neihgbor.elevation[index][0] + neihgbor.elevation[index][2] - 2 * neihgbor.elevation[index][
                                    1]) / 90.0
                            else:
                                theta_a = 0.0
                        else:
...
def predict(user):
    if random.random() < user.epsilon:
        ...
        for index,state in enumerate(user.old_state):
            ...
            if (x != -1) and (user.elevation[index][0] > PS.theta_access) :  
                state = np.array(state)  # 转为np.array类型
...
def learn(old_state,action,reward,new_state,beta,network):
    reward = reward / 10.0  
    priority = 1.0          
#    priority = old_state[0]     
    ...
    if network.replay_pool.full == 1:
        ...
        with tf.GradientTape() as tape:
            y_estimated = tf.reduce_max(network.evaluate_model(batch_state),axis=1)
            TD_error = y - y_estimated
            TD_error_hystetetic = tf.maximum(TD_error,beta * TD_error)
            loss = tf.reduce_mean(tf.square(TD_error_hystetetic))
        # if loss.numpy() > 10:
        #     pass
